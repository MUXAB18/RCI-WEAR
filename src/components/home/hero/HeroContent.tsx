  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeInUp} className="mb-6 lg:mb-8">
        <h1 className="font-sans text-[clamp(3.5rem,5vw,5rem)] font-bold leading-[1.05] tracking-tight text-near-black">
          Turn Your Ideas<br />
          into Premium<br />
          Apparel
        </h1>
      ...
      <motion.div variants={fadeInUp} className="mb-10 lg:mb-12">
        <p className="text-base md:text-lg text-near-black/70 max-w-[90%] lg:max-w-md font-medium leading-relaxed">
        ...
        <div className="flex -space-x-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="w-10 h-10 rounded-full border-2 border-white bg-gray-200 overflow-hidden relative">
              {/* Using standard img to avoid next/image domain configuration issues for placeholder avatars */}
              <img src={`https://i.pravatar.cc/100?img=${i + 10}`} alt="User avatar" className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
        <p className="text-xs text-near-black/60 font-medium max-w-[140px] leading-snug">
          Join <span className="font-bold text-near-black">100+ Brands</span> and start manufacturing now
        </p>
      </motion.div>
    </motion.div>
  );
