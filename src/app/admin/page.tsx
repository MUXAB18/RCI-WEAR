...
import { getOrderStats, getDashboardChartData } from '@/lib/api/order.service';
import { getReviewStats } from '@/lib/api/review.service';
import { DashboardCharts } from '@/components/admin/ui/DashboardCharts';
...
export default async function AdminDashboard() {
  const [projects, inquiries, orderStats, reviewStats, chartData] = await Promise.all([
    getAllProjects(),
    getAllInquiries(),
    getOrderStats(),
    getReviewStats(),
  ...
  return (
    <AdminShell>
      <div className="p-8 max-w-7xl">
          ...
          {stats.map((stat) => {
            ...
            return (
              <Link
                key={stat.label}
                href={stat.href}
                className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 xl:p-5 hover:bg-white/[0.06] hover:border-white/[0.12] transition-all group flex flex-col h-full"
              >
                <div className="flex flex-wrap items-start justify-between gap-2 mb-auto">
                  <div className={`shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br ${stat.color === 'blue' ? 'from-blue-500/20 to-blue-600/10' :
                  ...
                  {stat.change && (
                    <div className={`shrink-0 whitespace-nowrap flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-medium ${stat.trend === 'up' ? 'bg-green-500/10 text-green-400' :
                ...
                <div className="mt-6 flex flex-col">
                  <div className="text-2xl xl:text-3xl font-bold text-white mb-1 truncate">{stat.value}</div>
                  <div className="text-white/40 text-sm truncate">{stat.label}</div>
                  <div className="text-white/30 text-xs mt-1 min-h-[16px] truncate">
                    {stat.subtitle || '\u00A0'}
                  </div>
                </div>
              </Link>
            ...
            {quickActions.map((action) => {
              ...
              return (
                <Link
                  key={action.title}
                  href={action.href}
                  className="group relative bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 xl:p-5 hover:bg-white/[0.06] hover:border-white/[0.12] transition-all overflow-hidden"
                >
        ...
        {/* Dashboard Charts */}
        <DashboardCharts areaData={chartData.areaData} pieData={chartData.pieData} />
          ...
          <div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl overflow-hidden">
              {projects.slice(0, 5).map((project, i) => (
                <div
                  key={project.id}
                  className={`flex items-center gap-4 px-5 py-4 hover:bg-white/[0.03] transition-colors ${i < projects.slice(0, 5).length - 1 ? 'border-b border-white/[0.06]' : ''
                    }`}
                >
                  <img
          ...
          <div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl overflow-hidden">
              {inquiries.slice(0, 5).map((inquiry, i) => (
                <div
                  key={inquiry.id}
                  className={`px-5 py-4 hover:bg-white/[0.03] transition-colors ${i < inquiries.slice(0, 5).length - 1 ? 'border-b border-white/[0.06]' : ''
                    }`}
                >
                  <div className="flex items-start justify-between gap-4 mb-2">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${inquiry.status === 'new' ? 'bg-green-500/20 text-green-300' :
